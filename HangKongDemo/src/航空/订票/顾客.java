...
public class 顾客 extends JFrame implements ActionListener
{
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		{			
			...
			ticketnumber=adultnumber+childnumber;
					...
					if(i!=-2)
					{
						if(i==-1)
						{
							...
							try
							{
								...
								ResultSet rs = stmt.executeQuery("select adultFare,childFare from Plane where flight='"+string[4]+"'");
								while(rs.next())
								{
							...
							航空.clientFrame.setVisible(false);
							航空.clientFrame.dispose();
							航空.frame.setVisible(true);
							写入文件  writetofile=new 写入文件(string);
						}
						else
						{
				...
				//double
				else //如果(string[5].toString().trim().equals("往返")||string[5].toString().trim().equals("往返"))
				{
					i=seatinformation.dingPiao(string[4],string[3],ticketnumber);
					...
					else
					{
						...
						try
						{
							Connection con = DriverManager.getConnection("jdbc:sqlserver://localhost:1433;database=HangKongDB","sa","122026");
							Statement stmt = con.createStatement();
							ResultSet rs = stmt.executeQuery("select adultFare,childFare from Plane where flight='"+string[11]+"'");
							while(rs.next())
							{
								adultfare=rs.getFloat(1);
								childfare=rs.getFloat(2);
