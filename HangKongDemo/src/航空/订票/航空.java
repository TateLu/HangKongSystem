...
public class 航空 extends JFrame implements ActionListener
{
...
//			boxadult.addItem(adult[i]);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(e.getSource()==jbtbutton)
		{
			...
			//System.out.println("sflight:"+sflight);
	...
	public int singleflight()
	{
		...
		else
		{
			try
			{
				...
				Connection con = DriverManager.getConnection("jdbc:sqlserver://localhost:1433;database=HangKongDB","sa","122026");
				Statement stmt = con.createStatement();
				ResultSet rs = stmt.executeQuery("select * from Plane");
				while(rs.next())				
					if(rs.getString(6).trim().equals(string[4]))
					flag=1;
				if(flag==1)
					{	
						
						String airfirm1=new String();
						String sql="select start,destination,airFirm from Plane where flight='"+string[4]+"'";
	...
	public int doubleflight()
	{
		...
		else
		{
			try
			{
				...
				ResultSet drs = stmt.executeQuery("select * from Plane");
				...
				{	
					...
					String sql="select start,destination from Plane where flight='"+string[11]+"'";
	...
	public int multipleflight()
	{
		...
		else
		{
			try
			{
				...
				Statement stmt = con.createStatement();
				//ResultSet rs = stmt.executeQuery("select * from flight");
				ResultSet mrs = stmt.executeQuery("select * from Plane");
				//flag=1 exists the flight 
				while(mrs.next())
				{
				...
				{	
					...
					String sql="select start,destination from Plane where flight='"+string[16]+"'";
					...
					if(mstartplace.equals(endplace))
					{
						...
						i=0;
						sql="select flight from Plane where start='"+startplace+"' AND destination='"+endplace+"'";
